...
int boxSingles(Square *** sudoku, Box ** boxes) {
  ...
  // loop through boxes
  for (i = 0; i < 9; i++) {
    // loop through the possible array
    for (j = 0; j < 9; j++) {
      count = 0;
      ...
      for (x = 0; x < 9; x++) {
        ...
        if (count == 2) {
          break; // if the square is unsolvable for the number then leave
        }
      }
...
Box ** createBoxes() {
  int x, y;
  Box ** boxes;
  boxes = malloc(sizeof(Box*)*9); // creating space for the 9 boxes in the Sudoku puzzle
...
int updateBoxes(Square *** sudoku, int row, int col) {
  ...
  for (x = 0; x < 9; x++) {
    if (box->squares[x]->possible[number - 1] == 0) {
      // found a new number to elimnate
      box->squares[x]->solvable -= 1;
